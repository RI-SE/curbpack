...
#   make test
#   make test SUITE=uv
#   sh testing/automation/suit-runner.sh all|SUITE|CASE|VARIANT
#
...
usage() {
	echo "usage: sh testing/automation/suit-runner.sh CASE|SUITE|all" >&2
}
...
suite_markdown() {
	printf '%s/testing/test_suites/%s.md\n' "$repo_root" "$1"
}
