...
func TriageMarkdown(rep Report, full bool) string {
	...
	if !full {
		...
		b.WriteByte('\n')
		fmt.Fprintf(&b, "Confirmed %d structural/reference checks · producer %d · external %d. (--full for all)\n\n",
		...
		if rep.ContradictedCount > 0 {
			b.WriteString("Next: resolve the listed discrepancies before relying on this report.\n\n")
		} else if rep.UnconfirmedGenuine > 0 {
			b.WriteString("Next: locate the missing evidence or correct its references, then review again.\n\n")
		} else {
			b.WriteString("Next: inspect the source evidence and confirm that the subject and scope match your intended use.\n\n")
		}
		b.WriteString("---\n")
