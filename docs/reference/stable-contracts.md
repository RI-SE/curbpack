...
Frozen ops (unchanged): `validate_delta`, `get_latest_failure`, `graph_summary`, `explain_packet`. See [coreward-bridge.md](../../vision/docs/coreward-bridge.md).
...
| `review --verify-chain` | Exclusive mode: `curbpack review --verify-chain <parent.json> <child.json>`. Exit **0** when both reports' `record_digest` values match independently recomputed digests and `child.parent_record_digest` equals the recomputed parent digest (all non-empty); exit **1** on empty digests, claim/recompute mismatch, or chain break; usage → **2**. No `--repo`/`--batch`/`--since`/`--packs`/`--json`/`--full`. |
...
| Method | `method_id` = `curbpack-review-method`; tip `method_version` = **1.3.0** ([`vision/docs/method/review-method-1.3.0.md`](../../vision/vision/docs/method/review-method-1.3.0.md)); must equal `review.MethodVersion` |
...
See also: [Strategy boundary](../../policies/strategy-boundary.md) · [Coreward bridge](../../vision/docs/coreward-bridge.md) · [Security model](../../vision/docs/security-model.md) · [Phase 6 kill-test](../../vision/docs/internal/phase6-kill-test.md) · [Review method 1.3.0](../../vision/vision/docs/method/review-method-1.3.0.md) · [Shared Frame](../../vision/docs/shared-frame.md) · [Substantial-modification screening (delivered)](../../vision/docs/internal/substantial-modification-screening.md)
