...
.PARAMETER Version
  Release tag (default from install-manifest.json / v0.5.5). Use 'latest' for newest.
...
try {
  ...
  Write-Host "Next:"
  Write-Host ""
  Write-Host "  cd /path/to/your/git/repo"
  Write-Host "  curbpack scan"
  Write-Host ""
  Write-Host "Not ready to use a product repository?"
  Write-Host "  curbpack demo"
  Write-Host ""
  Write-Host "Optional: curbpack doctor"
  Write-Host "After PATH loss: curbpack doctor --repair   (or: install.ps1 -Repair)"
  Write-Host "If curbpack is not found, open a new PowerShell window."
  Write-Host ""
  Write-Host $Claim
} finally {
