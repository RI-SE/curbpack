...
- **Review repository mode** — `curbpack review --repo [path]` (default `.`) triages governed documentation surfaces via the same classifier/method as received-pack review; `ReferencesOnly` + closure `digest_scope`; `Finding.source` + per-document `--since` decay; method [`docs/method/review-method-1.1.0.md`](docs/method/review-method-1.1.0.md) (1.0.0 retained). Pin stays `@v0.5.2`. Not certification.
