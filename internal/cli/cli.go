...
func usage() {
	...
	fmt.Fprintf(os.Stderr, "  review --repo [path] [--packs a,b]  In-repo ProsePaths triage (cold default house-policy)\n")
	...
	fmt.Fprintf(os.Stderr, "  review <pack-dir> [--json] [--full] [--since <prior.json>]\n")
	fmt.Fprintf(os.Stderr, "                                Received-pack triage; --since = report differ (exit codes unchanged)\n")
	fmt.Fprintf(os.Stderr, "  review --repo [path] [--packs a,b] [--json] [--edges <file>] [--full] [--since <prior.json>]\n")
	fmt.Fprintf(os.Stderr, "                                Repository-mode ProsePaths triage; --edges ingest-only with --json\n")
	fmt.Fprintf(os.Stderr, "  review --verify-chain <parent.json> <child.json>\n")
	fmt.Fprintf(os.Stderr, "                                Exclusive parent/child record digest chain check\n")
