...
func commandUsage(cmd string) {
	...
	case "check":
		fmt.Fprintf(os.Stderr, "Usage: curbpack check [--heal] [--score] [--packs a,b] [--json] [--as-of RFC3339|YYYY-MM-DD]\n")
	...
	case "review":
		...
		fmt.Fprintf(os.Stderr, "       curbpack review --verify-chain <parent.json> <child.json>\n")
		fmt.Fprintf(os.Stderr, "       curbpack review --batch <parent-or-pack-dir>…\n")
		fmt.Fprintf(os.Stderr, "  Offline triage and versioned artifact integrity checks (no git, no network).\n")
		...
		fmt.Fprintf(os.Stderr, "  --verify-chain checks parent/child record digests; exclusive (no --repo/--batch/--since/--packs/--json/--full/--edges).\n")
		fmt.Fprintf(os.Stderr, "  --since shows NEW / CLEARED / PERSISTING only with matching complete method and surface scope identities; exit codes are unchanged.\n")
