...
- Pin: Action/consumers use `@v0.4.1` (prefer tag + commit SHA).
- **30-day trust-surface freeze continues from `v0.4.0` through `v0.4.1`** (activation polish only; no trust-surface rewrite): Action binary resolve, `SafeJoin` / pack path jail, attest OCC / `--allow-dirty` honesty, claim-safety, and explain-packet airlock — bugfixes only; no new trust-surface features.
