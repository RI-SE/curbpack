...
|---|---|---|
| Redaction / reader contracts | Unique failed/evaluated/skipped counts, explicit custom-home handling and compatible trend checks are implemented. Remaining crossing-format and native-platform acceptance is listed in the audit. | [repair audit](production-hardening-audit.md), [schemas](../schema/) |
| Canonical evaluation and receipt | V2 binds explicit `as_of`, exact pack/input/method identity and a digest-addressed cache; operational fields stay in receipts. Legacy adapters and separate emission/read regressions are present. Final acceptance remains open. | [W2 contract and tests](production-hardening-audit.md#w2-continuation-after-the-repair-checkpoint) |
| Persistence and concurrency | Packs stage as a set and publish a verified completion manifest last; caches use immutable objects and an authoritative pointer. Interrupted renames and hostile filesystem races remain distinct limitations. | [batch writer](../internal/outwrite/batch.go), [manifest review](../internal/review/pack_audit.go) |
...
| Independent trust assessment | Offline integrity, authenticity, completeness and applicability are separate typed results. Signer trust and recipient applicability are not established by manifest hashes; independent review remains required. | [audit engine](../internal/review/pack_audit.go), [bind resolution](../internal/attest/bind.go) |
...
|---|---|---|
| 1 | CUR-01: release/advertisement evidence, resource checks, static homepage CSS | Merged corrections in [#53](https://github.com/RI-SE/curbpack/pull/53); all 17 public-asset tests, including Chrome checks, passed in this run. |
| 2 | Action execution: reject consumer-controlled source builds; treat inputs as shell/JavaScript data | Merged in [#54](https://github.com/RI-SE/curbpack/pull/54); actual resolver regressions passed. The published Action pin remains human-controlled. |
| 3 | Contained, staged output writes; exclusive writers and recovery; Windows paths | Critical repairs implemented on the repair branch, with complete-set publication. Native interruption/platform acceptance remains open. |
| 4 | W2 canonical evaluation/receipt split, explicit `as_of`, complete identity, versioned cache; producer and reader determinism | Implemented on the repair branch; final matrix and review remain open ([audit](production-hardening-audit.md)) |
| 5 | Redaction, truthful tallies, comparable trends, conformity fields and compatibility | Implemented repairs and compatibility tests are on the repair branch; remaining crossing-format checks are listed in the audit. |
| 6 | Offline bundle schema/integrity validation and separate trust results | Implemented in the existing review engine; altered/missing/schema/symlink cases and historical report digests tested. Authenticity and applicability are explicitly not inferred. |
