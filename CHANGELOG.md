...
- **First-run scan honesty** — `curbpack scan` footer is `Next (optional):` so continuation is not read as required; Satisfied vs Open sections; exit 0 framed as diagnosis finished (not pass/cert). Installers warn/refuse non-`RI-SE/curbpack` `CURBPACK_REPO` unless `CURBPACK_REPO_I_UNDERSTAND=1`, and print RI-SE + do-not-use-afelin guidance on download/checksum 404. RISE tryout docs: stop after scan; no Discussions CTA (not enabled). Gate docs accept `Next:` or `Next (optional):` until next binary. Pin stays `@v0.5.2`. Not certification.
