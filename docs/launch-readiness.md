...
If still `NOASSERTION`, wait for GitHub re-detect (pin `@v0.5.0` already shipped with license hygiene).
...
- Market promise: a stranger’s **first green &lt;10 minutes** on pin `@v0.5.0` (safe try / product repo / CI).
...
| Day 30 | Freeze review: renew, narrow, or next `v0.4.x` bugfix only (`v0.5.0` = current instrument-panel pin) |
...
| Interim stance (until due) | **Renew freeze**; pin stays `@v0.5.0`; **no `v0.4.4`** cut; **no pack catalog unlock** |
